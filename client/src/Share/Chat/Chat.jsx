...
const socket = io("https://e-commerce-service-rghn.onrender.com/", {
...
function Chat(props) {
  ...
  const storedRoomId = localStorage.getItem("njs_asm3_roomId");
  const [roomId, setRoomId] = useState(storedRoomId ? storedRoomId : "");
  ...
  const onChat = async () => {
    if (!roomId) {
      ...
      if (newRoomData.room && newRoomData.room._id) {
        setRoomId(newRoomData.room._id);
        localStorage.setItem("njs_asm3_roomId", newRoomData.room._id);
      } else {
        console.error("Error creating a new room");
      }
    }
  ...
  useEffect(() => {
    const fetchDataAndSetLoad = async () => {
      try {
        if (roomId) {
          await fetchData();
          setLoad(false); // Set load to false after successfully fetching data
        }
      } catch (error) {
        console.error("Error fetching data:", error);
        setLoad(false); // Set load to false even if there's an error to prevent infinite loop
      }
    };
